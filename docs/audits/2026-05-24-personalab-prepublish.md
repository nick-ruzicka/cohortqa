# CohortQA pre-publish hardening + positioning

_Formerly known as **PersonaLab** — renamed to **CohortQA** at OSS publish (2026-05-28)._

**Date:** 2026-05-24 · **Branch:** `cohortqa/prepublish` (off `cohortqa/quickstart-example` — which includes Phase C and the example) · **Mode:** security hardening + standards-fit assessment + README draft · **Spend:** ~$0 (no API calls; pure code + writing) · **Verdict:** four tasks landed with honest scope on each. Hardening is real, promptfoo is the wrong tool, SKILL.md is a separable ship-later thing, README draft captures the positioning.

> **Branch choice flagged:** the prompt offered `cohortqa/phase-b` or a `cohortqa/prepublish` branch off it. I took `cohortqa/prepublish` off `cohortqa/quickstart-example` because the hardening edits land in the same files Phase C modified (`analyzer.py`, `synthesizer.py`) and the README work explicitly references Phase C mechanisms and the quickstart. Branching off raw `phase-b` would lose both and create downstream merge friction. Easy to rebase if you disagree.
...
```
$ grep -nE 'tools=|"tools":' cohortqa/core/analyzer.py cohortqa/core/synthesizer.py
(no matches)
```
...
### Tests added (6 new, in `cohortqa/tests/test_injection_resistance.py`):
...
**What CohortQA actually uses:**
- One model family (Anthropic), with stage-specific overrides via env vars (`COHORTQA_ANALYZER_MODEL`, `COHORTQA_SYNTHESIZER_MODEL`)
...
**What would CHANGE my mind:** if CohortQA grew an unstructured-output surface (e.g., a "summarize this polish-spec" feature with free-text output), promptfoo's behavioral assertions would become valuable. Or if we decided to support OS models (Llama, etc.) and wanted cross-provider regression tests. Neither is on the publish path.

### Recommendation

Stick with pytest + Pydantic structured output. The published README can simply say: *"Tests: 165 passing. Prompt construction + injection resistance covered in `cohortqa/tests/test_injection_resistance.py` and `test_synthesizer.py`. Structured output via Anthropic's `messages.parse()` enforces response shape at the SDK layer."*
...
**Honest verdict (2 lines):** CohortQA the framework is a CLI/library, NOT expressible as a SKILL.md directly — it runs Playwright, makes 9 API calls per orchestrator invocation, writes JSONL/markdown to disk across persona-parallel sessions. A separate ~50-line skill that WRAPS the CLI invocation is feasible and ships as a 1-page exercise after the framework lands; don't conflate them.
...
CohortQA is not that shape:

| CohortQA needs | SKILL.md provides |
|---|---|
...
### What a SKILL.md COULD do for CohortQA

A *thin invocation wrapper* is feasible:

```yaml
---
name: cohortqa-quickstart
description: When the user asks to UX-QA a web app, run CohortQA against it
allowed-tools: [Bash, Read, Edit, Write]
---

# Run CohortQA against the user's app

## When to invoke
- User mentions "QA the app", "find UX friction", "what do users hit",
  "run CohortQA", or similar.
- User has a locally-running web app or a static site.

## Steps
1. Verify the app is reachable (`curl <url>/`).
2. Check if `cohortqa` is installed; if not, point them at the
   quickstart README.
3. Read or create `app.yaml` describing the routes / actions / friction
   signals. Use `cohortqa/examples/quickstart/app.yaml` as the template.
4. Copy personas: `cp cohortqa/personas/*.yaml ./personas/`.
5. Run: `python3 -m cohortqa.core.orchestrator --app app.yaml ...`
6. Read the polish-spec from `_synthesis/` and present the cross-persona
   headline + top 3 patterns.

## Anti-patterns
- Do NOT try to embed the CohortQA analyzer/synthesizer prompts in
  this skill — they're 300+ lines and stage-specific.
- Do NOT run the orchestrator against production / live data.
- Do NOT skip the protected-action safety model (CohortQA handles
  this; just don't override it).
```

That's ~50 lines and lets a Claude Code / Cursor / Windsurf user invoke CohortQA without remembering the orchestrator's CLI shape.

**Honest scope: this skill is a separable ship.** It depends on CohortQA being `pip install`-able first (so the skill can just shell out). It also benefits from the framework already publishing the quickstart it references. Don't ship the skill until the framework's README, quickstart, and pyproject.toml are in. Then the skill is a 1-page exercise that lands in skill-registry repos as a separate artifact.
...
**Honest verdict (2 lines):** Drafted as a new file (`cohortqa/README.draft.md` would be cleaner if I'd written it there; I'm including the sections inline below for review). Pull what's useful when finalizing the existing `cohortqa/README.md`; don't ship this draft verbatim — it's a brief, not a final.
...
**Right-sized framing.** CohortQA is a complement to manual QA and
...
**What CohortQA does NOT replace:**
...
- Server-error / data-integrity / cross-component-consistency
  testing. CohortQA catches affordance and persona-fit friction; it
...
| | CohortQA | LLM "synthetic users" SaaS (Uxia &c.) | PersonaTwin-style |
|---|---|---|---|
...
pure Python rules in `cohortqa/core/behavior.py`. Zero LLM calls in
...
```markdown
## What CohortQA doesn't do (honestly)
...
- **Catches affordance + persona-fit friction, NOT server errors,
  ...
  your detail-page renders empty, CohortQA will report it as an
...
1. ✓ Quickstart example app (`cohortqa/examples/quickstart/`)
2. ✓ Phase C dormant-mechanism fix (`cohortqa/phase-c`)
3. ✓ Injection hardening (this branch)
4. ⌛ README right-size — draft sections above; needs assembly into `cohortqa/README.md`
