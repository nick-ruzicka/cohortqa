...
def render_polish_spec(app_config: dict[str, Any], spec: PolishSpec,
    ...
    if reprompted:
        lines.append("_⚠️ patterns recovered via re-prompt fallback "
                      "(initial call returned prose-only summary)_")
    lines += [
        "",
...
class Synthesizer:
    ...
    def synthesize(self, reports: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        ...
        # Re-prompt fallback: if the model wrote a summary that references
        # pattern groups but returned 0 structured FrictionPattern entries,
        # the model did the synthesis work in prose but bailed on structuring.
        # Make a second call asking it to convert the prose to entries.
        if len(spec.patterns) == 0 and _summary_references_groups(spec.overall_summary):
            logger.warning(
                "Synthesizer returned 0 patterns but summary references groups — "
                "triggering re-prompt fallback"
            )
            reprompt_spec = self._reprompt_for_patterns(spec.overall_summary)
            if reprompt_spec is not None and len(reprompt_spec.patterns) > 0:
                logger.info(
                    "Re-prompt recovered %d patterns", len(reprompt_spec.patterns)
                )
                spec = PolishSpec(
                    overall_summary=spec.overall_summary,
                    patterns=reprompt_spec.patterns,
                )
                reprompted = True
            else:
                logger.warning(
                    "Re-prompt also returned 0 patterns — proceeding with empty spec"
                )
    ...
    def _reprompt_for_patterns(self, summary: str) -> PolishSpec | None:
        """Second-chance call: ask the model to convert a prose summary
        into structured FrictionPattern entries."""
        reprompt_msg = (
            "Your previous synthesis produced an overall_summary that describes "
            "concrete friction groups, but the patterns list was empty. Convert "
            "the groups described below into FrictionPattern entries.\n\n"
            "Each pattern must include all required fields: title, signal_type, "
            "severity_range, personas_affected, description, proposed_fix, "
            "implementation_approach, estimated_effort, evidence, confidence.\n\n"
            "If a group is genuinely unstructured or lacks enough evidence, "
            "omit it rather than fabricating fields.\n\n"
            f"## Summary to structure\n\n{summary}"
        )
        try:
            response = self.client.messages.parse(
                ...
                messages=[{"role": "user", "content": reprompt_msg}],
                output_format=PolishSpec,
            )
            result = response.parsed_output
            if isinstance(result, PolishSpec):
                return result
        except Exception:
            logger.exception("Re-prompt call failed")
        return None
...
# Regex pattern: words that suggest the summary describes structured groups
# of findings rather than a single prose observation.
_GROUP_INDICATORS = re.compile(
    r"\b("
    r"first\s+group|second\s+group|third\s+group"
    r"|group\s+\d|group\s+of"
    r"|cluster|pattern(?:s)?"
...
def _summary_references_groups(summary: str) -> bool:
    """Return True if the summary text references 3+ group-like terms,
    suggesting the model did the synthesis work in prose but didn't
    populate the structured patterns list."""
    if not summary:
        return False
    matches = _GROUP_INDICATORS.findall(summary)
